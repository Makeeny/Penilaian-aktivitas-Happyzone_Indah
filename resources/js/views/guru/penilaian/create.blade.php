...
{{-- Section untuk custom CSS yang sudah disesuaikan --}}
@section('styles')
<style>
...
<div class="form-container-custom">
    ...
    <div class="form-card-custom">
        <div class="card-title-bar">
            {{-- Judul di dalam kartu form --}}
            <h3>Formulir Penilaian</h3>
            {{-- Tombol Kembali --}}
            <a href="{{ route('guru.penilaian.index', $mapel) }}" class="back-button-custom">
        ...
        <form action="{{ route('guru.penilaian.store.bulk') }}" method="POST">
            @csrf
            <div class="form-grid-custom">
                {{-- Detail Siswa dan Pelajaran --}}
                ...
                <div class="form-group-custom">
                    ...
                    <select name="subject_id" id="subject_id" class="form-control-custom" required>
                        @foreach ($mapels as $item)
                            <option value="{{ $item->id }}" {{ $item->id == $mapel->id ? 'selected' : '' }}>{{ $item->nama_mapel }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="form-group-custom">
                ...
                <div class="form-group-custom">
                    ...
                    <input type="date" name="tanggal" id="tanggal" class="form-control-custom" value="{{ date('Y-m-d') }}" required>
                </div>

                {{-- NOTE: Info box untuk bobot kriteria ditambahkan di sini --}}
                ...
                @foreach($kriterias as $kriteria)
                <div class="form-group-custom">
                    <label for="penilaian_{{ strtolower($kriteria->kode_kriteria) }}">{{$kriteria->nama_kriteria}} ({{$kriteria->kode_kriteria}})</label>
                    {{-- NOTE: Input field baru untuk nilai 1-100 --}}
                    <input type="number"
                           id="nilai_{{ strtolower($kriteria->kode_kriteria) }}"
                           name="nilai_mentah[{{$kriteria->id}}]"
                           class="form-control-custom nilai-kriteria"
                           placeholder="Nilai 1-100"
                           min="0" max="100" required
                           {{-- NOTE: Data atribut untuk menyimpan info kriteria --}}
