...
<head>
    ...
    <title>@yield('title', 'Dashboard siswa') - Happy Zone</title>
    ...
    /* Jika ada grid guru di halaman dashboard, ubah jadi 2 kolom */
    .guru-grid {
...
@media (max-width: 576px) {
    ...
    /* Jika ada grid guru, ubah menjadi 1 kolom */
    .guru-grid {
...
<body>
    <div class="wrapper">
        ...
        <ul class="nav-list">
            ...
            <li class="nav-item {{ request()->is('siswa/penilaian*') ? 'active' : '' }}">
            {{-- Menu akan aktif jika URL saat ini adalah /feedback --}}
            <li class="nav-item {{ request()->is('feedback*') ? 'active' : '' }}">
                {{-- Mengarahkan ke route 'feedback.create' yang sudah kita buat --}}
                <a href="{{ route('feedback.create') }}" class="nav-link">
                    <i class="fas fa-comment-dots"></i> {{-- Mengganti ikon agar lebih sesuai --}}
                    <span>Feedback Orang Tua</span>
                </a>
            </li> 
                
           {{-- Mengambil data mata pelajaran pertama sebagai default --}}
            @php
                $firstSubject = \App\Models\MataPelajaran::first();
            @endphp

            {{-- Menu akan aktif jika URL mengandung 'siswa/rekap-poin' --}}
            <li class="nav-item {{ request()->is('siswa/rekap-poin*') ? 'active' : '' }}">
                {{-- Pastikan ada mata pelajaran sebelum membuat link --}}
                @if($firstSubject)
                    <a href="{{ route('siswa.rekap-poin.show', $firstSubject->id) }}" class="nav-link">
                        <i class="fas fa-gift"></i> {{-- Mengganti ikon agar lebih sesuai --}}
                        <span>Penukaran Poin</span>
                    </a>
                @else
                    {{-- Tampilan jika tidak ada mata pelajaran sama sekali --}}
        ...
        <div class="main-content full-width" id="main-content">
            <header class="header">
                <div class="header-left">
                    ...
                    <a href="{{ route('siswa.dashboard') }}" class="header-logo" id="header-logo" style="text-decoration: none;">
                        <img src="{{ asset('images/logohappyzone.png') }}" alt="Logo">
                        <span class="header-title">Happy Zone</span>
                    </a>
                </div>
                <nav class="header-nav">
                    <a href="{{ route('siswa.notifications.index') }}"><i class="fas fa-bell"></i></a>
    ...
    {{-- ========================================================== --}}
    {{-- == JAVASCRIPT BARU UNTUK MENGONTROL POP-UP == --}}
    {{-- ========================================================== --}}
    <script>
